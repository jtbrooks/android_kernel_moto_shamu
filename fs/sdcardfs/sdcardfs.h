...
#define fix_derived_permission(x)	\
	do {						\
		(x)->i_uid = KUIDT_INIT(SDCARDFS_I(x)->d_uid);	\
		(x)->i_gid = KGIDT_INIT(get_gid(SDCARDFS_I(x)));	\
...
static inline void sdcardfs_copy_and_fix_attrs(struct inode *dest, const struct inode *src)
{
	...
	dest->i_uid = KUIDT_INIT(SDCARDFS_I(dest)->d_uid);
	dest->i_gid = KGIDT_INIT(get_gid(SDCARDFS_I(dest)));
